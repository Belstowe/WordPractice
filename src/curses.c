...
void Play (int *lvl, int *var) {
    if (*lvl == 0) Difficult(&*lvl);
    if (*var == 0) Variant(&*var);
    if (*var == 1) eng(&*lvl, &*var);
    if (*var == 2) rus(&*lvl, &*var);
}
...
void Menu(int *item) {
    ...
    *item = 0;
    printw("\t\t  МЕНЮ\n");
    printw("Добро пожаловать в Word Practice, программу для заучивания иноязычных слов\n\n"
           "Выберите :\n");
    printw("\t\t1. Начать!.\n");
    printw("\t\t2. Выход\n");
    do {
